(function () {
    ...
    (function ($) {
        ...
        function init_vue() {
            ...
            FWP.vue = new Vue({
                ...
                methods: {
                    ...
                    checkApi: function checkApi(testTypes, apikey) {
                        var this$1$1 = this;


                        var statuses = {};
                        var doTests = testTypes.split(",");

                        var updateStatus = function ( test, message, status, action ) {
                            if ( status === void 0 ) status = 'none';
                            if ( action === void 0 ) action = 'none';

                            var testEl = document.querySelector('.facetwp-api-status .test-' + test);
                        ...
                        var prompt = FWP.__('Each test will count as one billable API request in your Google Cloud account. Continue?');
                        if (confirm(prompt)) {
                        ...
                        testFunctions.checkPlaces = async function () {
                            updateStatus( 'places', FWP.__('Checking Places API (New) ...'), 'testing', 'testing' );                        
                            var placeUrl = "https://places.googleapis.com/v1/places/ChIJj61dQgK6j4AR4GeTYWZsKWw?fields=id,displayName,formattedAddress&key=" + apikey;

                            try {
                                var response = await fetch(placeUrl);
                                if (!response.ok) {
                        ...
                        testFunctions.checkPlaceslegacy = async function () {
                            updateStatus( 'placeslegacy', FWP.__('Checking Places API (Legacy) ...'), 'testing', 'testing' );                        
                            
                            this$1$1.loadGmaps(apikey);

                            var ref = await google.maps.importLibrary("places");
                            var PlacesService = ref.PlacesService;
                        ...
                        testFunctions.checkMaps = async function () {

                            updateStatus( 'maps', FWP.__('Checking Maps JavaScript API ...'), 'testing', 'testing' );    

                            this$1$1.loadGmaps(apikey);

                            var ref = await google.maps.importLibrary("maps");
                            var Map = ref.Map;
                        ...
                        var validTests = [ 'maps', 'places', 'placeslegacy', 'geocoder' ];
                        validTests.forEach(function (t) {
                            if ( doTests.includes(t) ) {
                                var tn = "check"+t.charAt(0).toUpperCase()+t.slice(1);
                                testFunctions[tn]();
                            } 
                        });
                    ...
                    loadGmaps: function loadGmaps(apikey) {                   
                    ...
                    resetCheckApi: function resetCheckApi() {
                        var prompt = FWP.__('FacetWP\'s settings page needs to reload to test a different Google Maps API key. Unsaved changes to other settings will be discarded. Continue?');
                        if (confirm(prompt)) {
        ...
        function init_custom_js() {
            ...
            $().on('click', '.export-all', function (e) {
                var $opts = $('.export-items').next('.fs-wrap');
                var selectedType = "undefined" != typeof e.target.dataset.value ? e.target.dataset.value+"-" : "";
            ...
            $().on('click', '.export-none', function (e) {
                var $opts = $('.export-items').next('.fs-wrap');
                $opts.find('.fs-option.selected').each(function() {
                    var $opt = $(this);
                    $opt.trigger('click');
