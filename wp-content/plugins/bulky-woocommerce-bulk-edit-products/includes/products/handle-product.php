...
class Handle_Product {
	...
	public function __construct() {
		...
		$user_meta_fields     = get_user_meta( $user_id, 'vi_wbe_product_meta_fields', true );
		$user_taxonomy_fields = get_user_meta( $user_id, 'vi_wbe_product_taxonomy_fields', true );

		$bulky_products        = Products::instance();
		$this->fields          = $bulky_products->get_final_fields();
		$this->meta_fields     = empty( $user_meta_fields ) ? get_option( 'vi_wbe_product_meta_fields' ) : $user_meta_fields;
		$this->taxonomy_fields = empty( $user_meta_fields ) ? get_option( 'vi_wbe_product_taxonomy_fields' ) : $user_taxonomy_fields;
	}
						...
						switch ( $meta_type ) {
							...
							case 'gallery':
								if ( is_array( explode( ',', $data ) ) ) {
									$data = explode( ',', $data );
									$image_ids = array_filter( $data, function ( $value ) {
										return ! is_null( $value ) && $value !== false && trim( $value ) !== '';
									} );
									$data      = $image_ids;
								}
								break;
		...
		switch ( $type ) {
			...
			case 'tags':
				...
				if ( $tax_object && isset( $tax_object->meta_box_sanitize_cb ) ) {
					$terms_translated = call_user_func_array( $tax_object->meta_box_sanitize_cb, array(
						$taxonomy,
						$terms
					) );
				...
				if ( ! empty( $meta_fields ) && is_array( $meta_fields ) && in_array( $type, array_keys( $meta_fields ) ) ) {
					...
					if ( $data_type ) {
						switch ( $data_type ) {
							...
							case 'image':
								$value = Helper::is_url($value) ? esc_url_raw( $value ) : $value;
								break;
							case 'gallery':
								$value = implode( ',', $value );
								break;
