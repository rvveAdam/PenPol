...
;(function (global, factory) {
    ...
    var jexcel = (function (el, options) {
        ...
        /**
         * Reset order arrow
         * Custom code by Hydro
         */
        obj.resetOrderArrow = function () {
            // Remove order
            for (var i = 0; i < obj.headers.length; i++) {
                obj.headers[i].classList.remove('arrow-up');
                obj.headers[i].classList.remove('arrow-down');
         ...
         * Custom code by Hydro
         */
        obj.orderAfterLoad = function () {
            for (var i = 0; i < obj.headers.length; i++) {
                let columnId = obj.headers[i].getAttribute('data-x');
                if ( obj.headers[i].classList.contains('arrow-up') ) {
                    obj.headers[i].classList.remove('arrow-up');
                    obj.orderBy(columnId, 1);
                    return;
                }
                if ( obj.headers[i].classList.contains('arrow-down') ) {
                    obj.headers[i].classList.remove('arrow-down');
                    obj.orderBy(columnId, 0);
                    return;
                }
            }
        }
            ...
            if (obj.options.lazyLoading == true) {
                if (jexcel.timeControlLoading == null) {
                    jexcel.timeControlLoading = setTimeout(function () {
                        ...
                        } else if ( obj.content.scrollTop < 10 ) { // obj.content.scrollTop <= obj.content.clientHeight(code of library) <=> obj.content.scrollTop < 10 ( code of hydro )
                            if (obj.loadUp()) {
                ...
                if (e.which == 37) {
                    ...
                    if ((e.ctrlKey || e.metaKey) && !e.shiftKey) {
                        if (e.which == 65) {
                            ...
                            // Ctrl + S
                            // jexcel.current.download(); Custom code by Hydro
                            e.preventDefault();
                        } else if (e.which == 89) {
                            // Ctrl + Y
                            jexcel.current.redo();
...
/******/ 	var __webpack_exports__ = __webpack_require__(827);
/******/ 	
/******/ })()
;